...
class AppointmentController {
  ...
  async store(req, res) {
    ...
    if (!checkIsProvider) {
      return res
        .status(401)
        .json({ error: 'You olny create appointment with providers' });
    }

    if (provider_id === req.userId) {
      return res
        .status(401)
        .json({ error: 'Dont permitted create appointment to youself' });
    }
  ...
  async delete(req, res) {
    ...
    if (isBefore(dateWithSub, new Date())) {
      return res.status(401).json({
        error: 'You cant only cancel appointment 3 hours in advance.',
      });
    }
